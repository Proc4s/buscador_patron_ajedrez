...
typedef struct {
	uint8_t	piezas;	// 4 bits altos => pieza origen, 4 bajos => pieza destino.
...
typedef struct {
	uint16_t	magic;	// magig number fijo a 0x55AA
	uint8_t	reser;	// reservado para futuro uso.
...
{
	movimientos[cabpartida.nmov] = mov; // anhade movimiento
	cabpartida.nmov++;	// actualiza numero movimientos.
}

// funcion para volcar a la base de datos binaria, la partida en curso ya recodificada.
void vuelcaPart(int fd)
{
	write(fd,&cabpartida,sizeof(cabpartida));	// escribe cabecera.
	write(fd,movimientos,cabpartida.nmov * sizeof(MOVBIN_t)); // escribe movimientos.
}
...
void eliminaPieza(uint8_t pieza,uint8_t posicion,TABLERO_t *tab)
{
	...
	switch(pieza & 0x7)
	{
		case REINA:
			...
			piezas->nreina--;	// Ajusta el numero de piezas activas.
			break;
		case TORRE:
...
void mueve(MOV_t *mov,TABLERO_t *tab)
{
	...
	movimiento.piezas = (mov->pieza << 4) | (mov->pieza & 0xf);
...
int posiblejaque(int rey,int pieza,int color,int indest,TABLERO_t *tab)
{
	...
	if(fila == filpieza) // pieza y rey en la misma fila.
	{
		...
		for(i=colum+incx;((i>= 0) && (i<8));i+=incx)
		{
			...
			tipo = tab->tab[fila*8+i];	// pieza que se encuentra en la casilla examinada.
			if(tipo != NADA)
			{
...
// funcion que realiza un enrroque largo. (enrroque de reina)
void enroquel(TABLERO_t *tab,uint8_t color)
{
	...
	if(color == NEGRA)
	{
		movimiento.piezas = ((TORRE | NEGRA) << 4) | ((TORRE | NEGRA) & 0xf);
		movimiento.origen = 0;
		movimiento.destino = 3;
		anadeMov(movimiento);
		movimiento.piezas = ((REY | NEGRA) << 4) | ((REY | NEGRA) & 0xf);
		movimiento.origen = 4;
	...
	{
		movimiento.piezas = (TORRE << 4) | (TORRE & 0xf);
		movimiento.origen = 56;
		movimiento.destino = 59;
		anadeMov(movimiento);
		movimiento.piezas = (REY << 4) | (REY & 0xf);
...
// funcion que realiza un enroque corto. (enrroque de rey)
...
void promoPieza(MOV_t *mov,TABLERO_t *tab)
{
	...
	movimiento.piezas = ((PEON | (mov->pieza & NEGRA)) << 4) | mov->pieza & 0xf;
	movimiento.origen = mov->org;
	movimiento.destino = mov->dest;
	anadeMov(movimiento);
...
void determov(uint8_t *pgn, uint8_t color,MOV_t *mov,TABLERO_t *tab)
{
	...
	{
		if(strcmp(pgn,"O-O-O") == 0) // enrroque largo
			enroquel(tab,color);
		else
...
//---------------
	...
	if(argc != 2)
	{
		fprintf(stderr,"Usage: %s <filebin> < fileorg\n",argv[0]);
		exit(1);
	}
	...
	if(fd < 0)
	{
		perror("Falla open salida");
		exit(2);
	}
	partidas = 0;
	linea = (uint8_t *)malloc(100*1024); // espacio de 100K para leer una linea, el maximo tamano es la linea de movimientos.

	while(fgets(linea,100*1024,stdin) != NULL)	// lectura linea a linea hasta que no haya mas.
	{
		if(memcmp(linea,"[Event",6) == 0)	// linea comienzo de partida.
		{
			partidas++;

			iniciaJuego(&tablero);	// inicia las estructuras del tablero virtual
			iniPart();					// inicia las estructuras binarias de salida.
			while(fgets(linea,100*1024,stdin) != NULL)	// sigue leyendo lineas.
			{
				if(memcmp(linea,"[Event",6) == 0)	// comienzo de nueva partida.
				{
					partidas++;
				}
				if(linea[0] != '1')	// no se trata de la linea de movimientos.
				{

					if(memcmp(linea,"[WhiteElo",9) == 0)	// linea de elo de blancas.
					{
						punte = strchr(linea,'"');	// el dato esta entre comillas.
						elo1 = atoi(punte+1);
					}
					else if(memcmp(linea,"[BlackElo",9) == 0)	// linea de elo de negras.
					{
						punte = strchr(linea,'"'); // el dato esta entre comillas.
						elo2 = atoi(punte+1);
					}
					else if(memcmp(linea,"[Result",7) == 0)	// linea de resultado de la partida.
					{
						if(strstr(linea,"1-0") != NULL)	// ganan blancas.
						{
							cabpartida.flags.ganablanca = 1;
							cabpartida.flags.gananegra = 0;
						}
						else if(strstr(linea,"0-1") != NULL)	// ganan negras.
						{
							cabpartida.flags.ganablanca = 0;
							cabpartida.flags.gananegra = 1;
						}
						else if(strstr(linea,"1/2") != NULL)	// tablas, anotamos que ganan ambos.
						{
							cabpartida.flags.ganablanca = 1;
							cabpartida.flags.gananegra = 1;
						}
					}
					else if(memcmp(linea,"[Termination",7) == 0)	// Terminacion de la partida.
					{
						if(strstr(linea,"Normal") != NULL)	// teminacion normal
				...
				while(1)
				{
					punte1 = strchr(punte,'{');	// buscamos comienzo anotaciones
					if(punte1 == NULL)	// no encuentra anotaciones.
						break;
					*punte1 = 0;	// termina el string en el comienzo de anotaciones.
					strcpy(tmp,punte);	// copia movimiento sin anotaciones a tmp.	
					punte1 = strchr(punte1+1,'}'); // apunta a la llave de cierre de anotaciones.

					punte1+=2;	// apunta a siguiente movimiento.
					
					npgn = atoi(tmp);		// numero de movimiento
					if(strstr(tmp,"...") != NULL)	// movimiento de negras
						color = 1;
					else
						color = 0;
					punte = strchr(tmp,' '); // apunta al blanco de separacion entre numero de movimiento y texto de este.
					punte++;						// apunta al texto del movimiento.
					punte2 = strchr(punte,' '); // apunta al final del texto del movimiento.
					*punte2 = 0;	// finaliza el string del texto del movimiento.
					...
					{
						if((*punte2 >= '0') && (*punte2 <= '9'))	// encontrada indicacion de fila.
							break;
						if((*punte2 == 'O') || (*punte2 == 'Q') || (*punte2 == 'R') | (*punte2 == 'N') || (*punte2 == 'B')) // encontrada indicacion de pieza o enrroque
							break;
						*punte2 = 0;	// eliminamos caracter.
					}
					strcpy(gpgn,punte);	// copiamos descripcion filtrada (eliminada cola descriptiva).
					movimientos++;
	...
	close(fd);	// cerramos fichero de base binaria generada.
	printf("\n");
}
