// Prueba de inserccion cassandra.
//
// carga el primer millon de partidas del fichero de entrada en formato
// pgn a la tabla partidas de la base cassandra.
//
#include <cassandra.h>
#include <stdio.h>
#include <stdlib.h>
...
void vuelcaPart(int fileid)
{
	...
	// movimientos.
	CassCollection *movcassandra = NULL;
	CassFuture* future = NULL;
	union {
		uint32_t val32;
		MOVBIN_t	movimiento;
	} valtmp;
	const char *query = "INSERT INTO ajedrez.partidas(fileid,particion,elomed,partidaid,ganador,movimientos) VALUES(?,?,?,?,?,?)";

	if(cabpartida.nmov == 0)
		return;
	movcassandra = cass_collection_new(CASS_COLLECTION_TYPE_LIST,cabpartida.nmov);
	for(i=0;i<cabpartida.nmov;i++)
	{
		valtmp.movimiento = movimientos[i];
		error = cass_collection_append_uint32(movcassandra,(cass_uint32_t)valtmp.val32);
		if(error != CASS_OK)
		{
			fprintf(stderr,"\nCASSAPPEND_ERR=>%d\n",error);
			exit(4);
	...
	cass_statement_bind_int16(statement,1,(partidas/1000000));
	cass_statement_bind_int16(statement,2,cabpartida.elomed);
	cass_statement_bind_int32(statement,3,cabpartida.ind);
	cass_statement_bind_int8(statement,4,ganador);
	cass_statement_bind_collection(statement, 5, movcassandra);		
	cass_collection_free(movcassandra);	
...
void main(int argc, char *argv[])
{
	...
	if(argc != 2)
	{
		fprintf(stderr,"Usage: %s <fileid> < fileorg\n",argv[0]);
		exit(1);
	}
	fileid = atoi(argv[1]);
	conectaCassandra();
